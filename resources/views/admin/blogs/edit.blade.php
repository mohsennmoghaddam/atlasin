    ...
    <div class="container mt-5" style="direction: rtl">
        ...
        <form action="{{ route('blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" name="title[fa]" class="form-control" value="{{ $blog->title['fa'] ?? '' }}" required>
            </div>
            <div class="form-group mb-3">
                <label>عنوان (انگلیسی)</label>
                <input type="text" name="title[en]" class="form-control" value="{{ $blog->title['en'] ?? '' }}" required>
            </div>

            {{-- توضیح کوتاه --}}
            <div class="form-group mb-3">
                <label>توضیح کوتاه (فارسی)</label>
                <textarea name="short_description[fa]" class="form-control">{{ $blog->short_description['fa'] ?? '' }}</textarea>
            </div>
            <div class="form-group mb-3">
                <label>توضیح کوتاه (انگلیسی)</label>
                <textarea name="short_description[en]" class="form-control">{{ $blog->short_description['en'] ?? '' }}</textarea>
            </div>
