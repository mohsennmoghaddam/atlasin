                        ...
                        <table class="table table-bordered table-hover text-center align-middle">
                            ...
                            @foreach($whyUses as $whyUse)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($whyUse->image)
                                            <img src="{{ asset('storage/' . $whyUse->image) }}" alt="image" width="80" class="rounded shadow-sm">
                                        @else
                                        ...
                                        <ul class="list-unstyled mb-0">
                                            @foreach($whyUse->items as $item)
                                                <li class="mb-1 d-flex align-items-center">
                                                    @if($item->icon)
                                                        <img src="{{ asset('storage/' . $item->icon) }}" alt="icon" width="20" class="me-2 rounded shadow-sm">
                                                    @endif
                                    ...
                                    <td>
                                        <form action="{{ route('why-uses.destroy', $whyUse) }}" method="POST" onsubmit="return confirm('آیا مطمئن هستید؟');">
                                            @csrf
                                            @method('DELETE')
