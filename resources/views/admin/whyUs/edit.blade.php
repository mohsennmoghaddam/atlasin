                    ...
                    <form action="{{ route('why-uses.update', $whyUse) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="title[fa]" value="{{ old('title.fa', $whyUse->title['fa'] ?? '') }}" class="form-control" required>
                            </div>
                            <div class="col-md-6">
                                <label>Title (English)</label>
                                <input type="text" name="title[en]" value="{{ old('title.en', $whyUse->title['en'] ?? '') }}" class="form-control" required>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <textarea name="description[fa]" class="form-control">{{ old('description.fa', $whyUse->description['fa'] ?? '') }}</textarea>
                            </div>
                            <div class="col-md-6">
                                <label>Description (English)</label>
                                <textarea name="description[en]" class="form-control">{{ old('description.en', $whyUse->description['en'] ?? '') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            @if($whyUse->image)
                                <img src="{{ asset('storage/' . $whyUse->image) }}" alt="image" width="120" class="rounded mb-2">
                            @else
                                ...
                                <div class="border rounded p-3 mb-3 bg-light item-block">
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-md-5">
                                            ...
                                            <input type="text" name="items[{{ $index }}][title][fa]" value="{{ $item->title['fa'] ?? '' }}" class="form-control" required>
                                        </div>
                                        <div class="col-md-5">
                                            <label>Item Title (English)</label>
                                            <input type="text" name="items[{{ $index }}][title][en]" value="{{ $item->title['en'] ?? '' }}" class="form-control" required>
                                        </div>
                                        <div class="col-md-2">
                                            <label>آیکون فعلی</label><br>
                                            @if($item->icon)
                                                <img src="{{ asset('storage/' . $item->icon) }}" width="40" class="mb-2 rounded shadow-sm">
                                            @endif
                        ...
                        <button type="button" class="btn btn-outline-secondary mb-3" onclick="addItem()">+ افزودن آیتم</button>
