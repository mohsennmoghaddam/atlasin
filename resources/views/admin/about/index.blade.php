                        ...
                        <table class="table table-bordered table-hover align-middle text-center">
                            ...
                            @foreach($abouts as $about)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($about->main_image)
                                            <img src="{{ asset('storage/' . $about->main_image) }}" width="70" height="50" class="rounded">
                                        @else
                                    ...
                                    <td>
                                        @if($about->call_us_image)
                                            <img src="{{ asset('storage/' . $about->call_us_image) }}" width="70" height="50" class="rounded">
                                        @else
                                            <span class="text-muted">ندارد</span>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ route('about.show', $about) }}" class="btn btn-sm btn-info">نمایش</a>
                                    </td>
                                    <td>
                                        <a href="{{ route('about.edit', $about) }}" class="btn btn-sm btn-warning">ویرایش</a>
                                    </td>
                                    <td>
                                        <form action="{{ route('about.destroy', $about) }}" method="POST" onsubmit="return confirm('آیا مطمئن هستید؟');">
                                            @csrf
                                            @method('DELETE')
