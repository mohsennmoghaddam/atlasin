                    ...
                    @foreach($categories as $category)
                        <tr>
                            ...
                            <td class="table-action">
                                <a href="{{route('categories.edit',$category)}}"><i class="tf-icon bx bx-edit menu-icon" data-feather="edit-2"></i></a>
                            </td>
                            <td>
                                <form action="{{route('categories.destroy',$category)}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <input type="submit" class="hidden-lg btn btn-sm btn-danger"  id="delete" value="حذف">
                                </form>
                            </td>
                        </tr>
                    @endforeach
