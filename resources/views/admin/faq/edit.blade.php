...
@section('content')
    <div class="container mt-4" dir="rtl">
        <h4 class="text-center mb-3">ویرایش سؤال متداول</h4>

        <form action="{{ route('faqs.update', $item) }}" method="POST" id="faq-form">
            @csrf @method('PUT')

            <div class="mb-3">
                <label class="form-label">کتگوری (اختیاری)</label>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="question[fa]" class="form-control" value="{{ $item->question['fa'] ?? '' }}" required>
                </div>
                <div class="col-md-6 mb-3" dir="ltr">
                    <label class="form-label">Question (EN)</label>
                    <input type="text" name="question[en]" class="form-control" value="{{ $item->question['en'] ?? '' }}" required>
                </div>
            </div>

            <div class="mb-3">
                <label class="form-label">پاسخ (FA)</label>
                <textarea id="answer-fa" name="answer[fa]" class="form-control" rows="6">{!! $item->answer['fa'] ?? '' !!}</textarea>
            </div>
            <div class="mb-3" dir="ltr">
                <label class="form-label">Answer (EN)</label>
                <textarea id="answer-en" name="answer[en]" class="form-control" rows="6">{!! $item->answer['en'] ?? '' !!}</textarea>
            </div>
