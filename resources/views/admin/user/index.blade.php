                            ...
                            @foreach($useres as $user)
                                <tr class="table-primary">
                                    ...
                                    <td class="table-action">
                                        <a href="{{route('role.edit', $user)}}"><i class="tf-icon bx bx-edit "></i></a>
                                    </td>
                                    <td>
                                        <form class="form-group" action="{{route('role.destroy', $user)}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            {{----}}
                                            {{--                                            @if(auth()->user()->role->hasPermission('delete-role'))--}}
                                            <button class="btn btn-danger" disabled type="submit" value="">حذف</button>
                                            {{--                                            @else--}}
                                    ...
                                    <td>
                                        ...
                                        <a href="{{route('user.show', $user->id)}}"><button class="btn btn-primary">مشاهده پروفایل کاربر</button></a>
                                        {{--                                        @else--}}
