...
@section('content')
    ...
    <section class="py-5 text-center p ">
        <div class="container">
            ...
            <p class="text-muted fs-5 justify-content-start" style="text-align: justify">{{ $body }}</p>
        </div>
    </section>
            ...
            <div class="carousel-inner">
                @foreach($sliders as $index => $slider)
                    <div class="carousel-item @if($index === 0) active @endif">
                        <div class="slider-container position-relative" style="height: 700px; overflow: hidden;">
                            <img src="{{ asset('storage/' . $slider->image) }}"
                                 class="w-100 h-100 object-fit-cover"
                                 alt="slider">

                            <div class="carousel-caption d-flex flex-column justify-content-center align-items-{{ app()->getLocale() === 'fa' ? 'center' : 'end' }} text-{{ app()->getLocale() === 'fa' ? 'start' : 'end' }}"
                                ...
                                <p class="text-white mb-4 fs-5 text-start">
            ...
            @foreach($cards as $card)
                ...
                <div class="col-12 col-sm-6 col-lg-3 mb-4">
                    <div class="card shadow h-100"
                         style="background-color: #007bff; color: white; border-radius: 10px;">
                        <div class="card-body d-flex align-items-center p-3" style="min-height: 100px;">
                            @if($locale === 'fa')
                                <img src="{{ asset('storage/' . $card->icon) }}" class="rounded-circle ms-3" width="100"
                                     height="100" alt="icon">
                            ...
                            @else
                                ...
                                <img src="{{ asset('storage/' . $card->icon) }}" class="rounded-circle ms-3" width="60"
                                     height="60" alt="icon">
                            @endif
                        </div>
                        @if($card->slug)
                            <div class="card-footer bg-transparent text-center border-0 pb-2">
                                <a href="{{ $card->slug }}" class="btn btn-light btn-sm px-3 py-1"
                                   style="font-size: 0.8rem;">{{ __('مشاهده محصولات') }}</a>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-lg-7">
                    ...
                    @if($item->catalog)
                        <a class="btn btn-outline-primary" target="_blank"
                           href="{{ asset('storage/'.$item->catalog) }}">
    ...
    @foreach($maskCategories as $cat)
        <section class="container my-5">
            <h5 class="text-center mb-4">{{ $cat->title[$loc] ?? '' }}</h5>
