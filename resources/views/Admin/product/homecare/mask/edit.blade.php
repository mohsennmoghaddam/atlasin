...
@section('content')
    <div class="container mt-4" dir="rtl">
        ...
        <form action="{{ route('homecare-mask-category.update', $category) }}" method="POST"
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="title[fa]" class="form-control" value="{{ $category->title['fa'] ?? '' }}"
                           required>
                </div>
                <div class="col-md-6 mb-3">
                    <label>Title (English)</label>
                    <input type="text" name="title[en]" class="form-control" value="{{ $category->title['en'] ?? '' }}"
                           required>
                </div>
            </div>
            ...
            @forelse($category->items as $idx => $it)
                <div class="d-flex align-items-center border rounded p-2 mb-2">
                    <img src="{{ asset('storage/'.$it->image) }}" class="rounded-circle me-3" width="70" height="70"
                         style="object-fit:cover">
                    <input type="hidden" name="items[{{ $idx }}][id]" value="{{ $it->id }}">
                    <div class="me-2">
                        <label class="mb-1 small">ترتیب</label>
                        <input type="number" name="items[{{ $idx }}][order]" class="form-control"
                               value="{{ $it->order }}" min="1">
                    </div>
                    <div class="form-check ms-3">
                        <input class="form-check-input" type="checkbox" name="items[{{ $idx }}][_delete]" value="1"
                               id="del{{ $it->id }}">
