...
@section('content')
    <div class="container mt-4" dir="rtl">
        ...
        <form action="{{ route('hospital-stages.update', $item) }}" method="POST" enctype="multipart/form-data"
              id="stage-form">
            @csrf @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <textarea id="title-fa" name="title[fa]" class="form-control"
                              rows="2">{{ $item->title['fa'] ?? '' }}</textarea>
                </div>
                <div class="col-md-6 mb-3" dir="ltr">
                    <label class="form-label">Title (EN)</label>
                    <textarea id="title-en" name="title[en]" class="form-control"
                              rows="2">{{ $item->title['en'] ?? '' }}</textarea>
                </div>
            </div>

            <div class="mb-3">
                <label class="form-label">متن (FA)</label>
                <textarea id="body-fa" name="body[fa]" class="form-control"
                          rows="6">{{ $item->body['fa'] ?? '' }}</textarea>
            </div>
            <div class="mb-3" dir="ltr">
                <label class="form-label">Body (EN)</label>
                <textarea id="body-en" name="body[en]" class="form-control"
                          rows="6">{{ $item->body['en'] ?? '' }}</textarea>
            </div>

            <div class="mb-3">
                <label class="form-label">تصویر</label>
                @if($item->image)
                    <div class="mb-2"><img src="{{ asset('storage/'.$item->image) }}" width="160"
