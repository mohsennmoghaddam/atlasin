                ...
                <div class="card shadow">
                        ...
                        <table class="table table-bordered table-hover align-middle text-center">
                            ...
                            @foreach($items as $row)
                                <tr>
                                    ...
                                    <td>
                                        @if($row->image)
                                            <img src="{{ asset('storage/'.$row->image) }}" width="120" height="70"
                  ...
                  <span class="badge {{ $row->is_active ? 'bg-success' : 'bg-secondary' }}">
                    {{ $row->is_active ? 'فعال' : 'غیرفعال' }}
                                    ...
                                    <td><a href="{{ route('hospital-schematics.edit', $row) }}"
                                           class="btn btn-warning btn-sm">ویرایش</a></td>
                                    <td>
                                        <form action="{{ route('hospital-schematics.destroy', $row) }}" method="POST"
                ...
                <div class="card shadow">
                  ...
                  <span class="badge {{ $row->is_active ? 'bg-success' : 'bg-secondary' }}">
                    {{ $row->is_active ? 'فعال' : 'غیرفعال' }}
                                    ...
                                    <td><a href="{{ route('hospital-summaries.edit', $row) }}"
                                           class="btn btn-warning btn-sm">ویرایش</a></td>
                                    <td>
                                        <form action="{{ route('hospital-summaries.destroy', $row) }}" method="POST"
                                              onsubmit="return confirm('حذف شود؟')">
                                            @csrf @method('DELETE')
                                            <button class="btn btn-danger btn-sm">حذف</button>
                                        </form>
            ...
            <div class="col-xl-11 mt-4">
                <div class="card shadow">
                    <div class="card-header d-flex justify-content-between align-items-center bg-danger text-white">
                        ...
                        <a href="{{ route('hospital-stages.create') }}" class="btn btn-light btn-sm"><i
                                    class="bx bx-plus-circle"></i> افزودن آیتم</a>
                    </div>
                    <div class="card-body table-responsive">
                        <table class="table table-bordered table-hover align-middle text-center">
                            <thead class="table-light">
                            ...
                            @foreach($stage as $row)
                                <tr>
                                    ...
                                    <td>{{ $row->category->title['fa'] ?? $row->category->title['en'] ?? $row->category->slug }}</td>
                                    <td>@if($row->image)
                                            <img src="{{ asset('storage/'.$row->image) }}" width="110" height="70"
                                    ...
                                    <td>{{ $row->order }}</td>
                                    <td>
                                        <span class="badge {{ $row->is_active?'bg-success':'bg-secondary' }}">{{ $row->is_active?'فعال':'غیرفعال' }}</span>
                                    </td>
                                    <td><a href="{{ route('hospital-stages.edit',$row) }}"
                                           class="btn btn-warning btn-sm">ویرایش</a></td>
                                    <td>
                                        <form action="{{ route('hospital-stages.destroy',$row) }}" method="POST"
                                              onsubmit="return confirm('حذف شود؟')">
                                            @csrf @method('DELETE')
                                            <button class="btn btn-danger btn-sm">حذف</button>
                                        </form>
                ...
                <div class="card shadow">
                                    ...
                                    <td>
                                        @if($row->file)
                                            <a class="btn btn-outline-primary btn-sm" target="_blank"
                                               href="{{ asset('storage/'.$row->file) }}">دانلود</a>
                                        @endif
                                    </td>
                                    <td>{{ $row->order }}</td>
                                    <td>
                  <span class="badge {{ $row->is_active ? 'bg-success' : 'bg-secondary' }}">
                    {{ $row->is_active ? 'فعال' : 'غیرفعال' }}
                                    ...
                                    <td><a href="{{ route('hospital-catalogs.edit', $row) }}"
                                           class="btn btn-warning btn-sm">ویرایش</a></td>
                                    <td>
                                        <form action="{{ route('hospital-catalogs.destroy', $row) }}" method="POST"
                                              onsubmit="return confirm('حذف شود؟')">
                                            @csrf @method('DELETE')
                                            <button class="btn btn-danger btn-sm">حذف</button>
                                        </form>
            ...
            <div class="col-xl-11 mt-4">
                <div class="card shadow">
                    ...
                    <div class="card-header d-flex justify-content-between align-items-center bg-danger text-white">
                        ...
                        <a href="{{ route('hospital-gallery_items.create') }}" class="btn btn-light btn-sm">
                        ...
                        <table class="table table-bordered table-hover align-middle text-center">
                            ...
                            @forelse($gallery as $i => $item)
                                <tr>
                                    ...
                                    <td>
                                        @if($item->image)
                                            <img src="{{ asset('storage/'.$item->image) }}" width="110" height="70"
                                                 class="rounded" style="object-fit:cover">
                                        @endif
                                    </td>
                                    <td>{{ $item->title['fa'] ?? '' }}</td>
                    ...
                    <span class="badge {{ $item->is_active ? 'bg-success' : 'bg-secondary' }}">
                      {{ $item->is_active ? 'فعال' : 'غیرفعال' }}
                                    ...
                                    <td>
                                        <a href="{{ route('hospital-gallery_items.edit', $item) }}"
                                           class="btn btn-sm btn-warning">ویرایش</a>
                                    </td>
                                    <td>
                                        <form action="{{ route('hospital-gallery_items.destroy', $item) }}" method="POST"
                                              onsubmit="return confirm('حذف شود؟')">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="text-muted">موردی ثبت نشده است.</td>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
