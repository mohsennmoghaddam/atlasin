                        ...
                        <table class="table table-bordered table-hover align-middle text-center">
                            ...
                            @foreach($sections as $row)
                                <tr>
                                    ...
                                    <td>{{ $row->order }}</td>
                                    <td><a href="{{ route('hospital-texts.edit', $row) }}"
                                           class="btn btn-warning btn-sm">ویرایش</a></td>
                                    <td>
                                        <form action="{{ route('hospital-texts.destroy', $row) }}" method="POST"
                        ...
                        <table class="table table-bordered table-hover align-middle text-center">
                            ...
                            @foreach($categories as $c)
                                <tr>
                                    <td>{{ $c->id }}</td>
                                    <td>@if($c->icon)
                                            <img src="{{ asset('storage/'.$c->icon) }}" width="70" height="70"
                                    ...
                                    <td>{{ $c->slug }}</td>
                                    <td>{{ $c->order }}</td>
                                    <td><a href="{{ route('hospital-category.edit',$c) }}"
                                           class="btn btn-warning btn-sm">ویرایش</a></td>
                                    <td>
                                        <form action="{{ route('hospital-category.destroy',$c) }}" method="POST"
                                              onsubmit="return confirm('حذف شود؟')">
                                            @csrf @method('DELETE')
                                            <button class="btn btn-danger btn-sm">حذف</button>
                                        </form>
