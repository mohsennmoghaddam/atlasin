        ...
        <form action="{{ route('about.update', $about->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="title[fa]" value="{{ json_decode($about->title)->fa ?? '' }}"
                           class="form-control" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Title (English)</label>
                    <input type="text" name="title[en]" value="{{ json_decode($about->title)->en ?? '' }}"
                           class="form-control" required>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="subtitle[fa]" value="{{ json_decode($about->subtitle)->fa ?? '' }}"
                           class="form-control" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Subtitle (English)</label>
                    <input type="text" name="subtitle[en]" value="{{ json_decode($about->subtitle)->en ?? '' }}"
                           class="form-control" required>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="final_paragraph[fa]" class="form-control"
                          rows="3">{{ json_decode($about->final_paragraph)->fa ?? '' }}</textarea>
            </div>
            <div class="mb-3">
                <label class="form-label">Final Paragraph (English)</label>
                <textarea name="final_paragraph[en]" class="form-control"
                          rows="3">{{ json_decode($about->final_paragraph)->en ?? '' }}</textarea>
            </div>
                ...
                @foreach($about->icons as $index => $icon)
                    <div class="icon-item border rounded p-3 mb-3 bg-light">
                        ...
                        <div class="mb-2">
                            ...
                            <img src="{{ asset('storage/' . $icon->icon_image) }}" alt="Icon" width="100"
                                 class="img-thumbnail">
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="text" name="icons[{{ $index }}][icon_title][fa]"
                                       value="{{ json_decode($icon->icon_title)->fa ?? '' }}" class="form-control">
                            </div>
                            <div class="col-md-6 mb-2">
                                <label>عنوان (en)</label>
                                <input type="text" name="icons[{{ $index }}][icon_title][en]"
                                       value="{{ json_decode($icon->icon_title)->en ?? '' }}" class="form-control">
                            </div>
                        </div>
                    </div>
                @endforeach
