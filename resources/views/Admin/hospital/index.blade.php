                ...
                @forelse($hospitals as $hospital)
                    <div class="col-md-4 mb-4">
                        <div class="card h-100 shadow-sm">
                            @if($hospital->image)
                                <img src="{{ asset('storage/' . $hospital->image) }}" class="card-img-top"
                                     alt="{{ $hospital->name }}">
                            @endif
                            <div class="card-body text-center">
                                ...
                                @if($hospital->website)
                                    <a href="{{ $hospital->website }}" class="btn btn-outline-primary btn-sm mb-2"
                                       target="_blank">
                            ...
                            <div class="card-footer text-center">
                                ...
                                <form action="{{ route('hospitals.destroy', $hospital) }}" method="POST"
                                      class="d-inline-block" onsubmit="return confirm('آیا مطمئن هستید؟')">
                                    @csrf
                                    @method('DELETE')
